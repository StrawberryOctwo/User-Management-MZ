...
const ViewTeacherPage: React.FC = () => {
  ...
  const loadTeacher = async () => {
    ...
    try {
      ...
      teacherData.availabilities = processAvailabilities(
        teacherData.availabilities
      );
  ...
  const formattedContractStartDate = teacher
    ? format(new Date(teacher.contractStartDate), 'PP')
    : '';
  const formattedContractEndDate = teacher
    ? format(new Date(teacher.contractEndDate), 'PP')
    : '';
        ...
        { field: 'createdAt', headerName: t('created_date'), flex: 1 },
        {
          ...
          renderCell: (params: { row: { id: any } }) => (
            <Button
              ...
              onClick={() =>
                window.open(
                  `/management/locations/view/${params.row.id}`,
                  '_blank'
                )
              }
            >
...

    {
      ...
      columns: [
        ...
        { field: 'path', headerName: t('path'), flex: 1 },
        {
          ...
          headerName: t('actions'),
          renderCell: (params: {
            row: { id: any; name: string; path: string };
          }) => (
